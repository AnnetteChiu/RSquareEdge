...
We see gender is coded. The docs (?normtemp) say 1 for male 2 for female. Here we split the data to perform a "t test":
...
For R's modeling functions, there is a syntax -- the  model formula -- that is usually designed for this:
...
The shortest -- by a hair -- is the use of `subset`. This is a convenience function. Rather than describe that function, we mention a similar one from  the `dplyr` package.

## Subsetting, Filtering data frames using `dplyr`

The `dplyr` package provides some higher-level interfaces to indexing. For this task, filtering our cases, we have the `filter` function.
...
```{r}
a <- select(Cars93, Manufacturer, Model, 
...
(lower case `filter` to filter rows, upper case to `Filter` columns, in a data frame.)
...
The last command could aslo have been done in one function call using base R's `subset` function:
...
(The popular `plyr` package has many similar functions distinguished by the data types: `ddply`, `adply`, ...)
...
Part of the task of `sapply` (the "s" part) is to *simplify* the output. 
...
The functions `sapply`, `Map`, `lapply`, ... are [higher-order](https://en.wikipedia.org/wiki/Higher-order_function) functions. For one of the arguments, we pass in  a function object by name (or anonymously). Often these functions are already defined for us (e.g., `is.numeric`), but there are times where we would want to define our own. For example, one way to remove NA values before computing the mean is to set the argument `na.rm=TRUE`. It is false by default. We could do this:
...
The `by_origin` variable is a list with two components: one for `USA` and one `non-USA`. If we wanted we could apply a function to each. 
