...
finding the mean or standard deviation of a group? There the model
formula is not defined. It is helpful to develop some basic data
massaging tools:
...
"Environment" tab. But as these come from built-in data sets, an extra
...
If you want to reduce typing, we can even define function
"anonymously":
...
and one `non-USA`. If we wanted we could apply a function to each.
...
data frame and returns the average. For that there is nothing built-in,
so we roll our own:
