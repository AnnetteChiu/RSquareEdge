...
The goal of today's notes is to go over one way to organize your work within `R`.

Let's begin with an example, how to use NYC's 311 open data base from within R.
...
There are alternatives, but here we will discuss how to use a URL to pre-filter the data we fetch into R
...
Basically, one can cook up a url that can be used to return matching records in JSON format. (Or others, such as XML or CSV).


Let's look at an example session (the `script.R` file in this directory)

## recent additions

Load some packages (`jsonlite` to parse JSON outputs; `dplyr` for manipulation of data frames; `lubridate` to work with dates; `magrittr` to tidy up some code):
...
We can easily create `POSIXct` times using `lubridate` functions:

```{r}
d2 = now()
d1 = d2 - days(2)  ## grab last two days
c(d1, d2)
```

These are converted into the time format Socrata wants as follows:
...
To convert this to a data frame, the *super* convenient `fromJSON` function works:
...
What kinda things to people call 311 about? We look at the `complaint_type` variable.
...
## get rid of complicated data frame variable -- it is rendundant here anyways:
...
Mapping is always a fun visualization. For mapping we have `RgoogleMaps`. Let's see if we can just borrow and adjust someone's [example](https://www.r-bloggers.com/heatmap-of-toronto-traffic-signals-using-rgooglemaps/)
...
A quick peek at GoogleMaps shows me that a center is basically:
...
From [usgs](https://www2.usgs.gov/faq/categories/9794/3022) we see that one minute of latitude at 38 degrees N is basially 1.15 miles. So a block is basically `1/60/15 =` `r 1/60/15`.
...
This was meant to simulate how one might work interactively at the command line or with a script. We bounce along thought to thought. While this isn't bad, it is helpful to go back over and think how we could do things more efficiently/easily.

Usually this involves creating functions.
...
We clearly see a template (the `d1` and `d2` values are all that are replaced.) When we use copy-and-paste to create new commands, just slightly modifying things we should be thinking: write  a function:
...
(There is also `strftime` that is designed for this conversion, but `sprintf` is of wider utility.)
...
Functions are defined by the `function` keyword. They have two visible things: a set of arguments, the body:
...
A third is an *environment* where variables within the body are found.


## Similarly

We repeated the following code:
...
(This one could be improved! -- but that takes us far a field.)

## arguments

In the past two example, we have functions that take a single argument, so the usage is pretty clear -- when the body refers to the variable it uses the value passed to the function when it is called.

Functions in `R` are not limited to single value, in fact many functions have *too* many arguments.
...
What does the `base = exp(1)` do? It provides a default if no base value is specified.
...
query_to_url <- function(query) {
   sprintf("%s?$where=%s", baseurl, query)
}
```
...
The is a distinction between the environment the function is defined in and the environment of the call. This can be of importance when functions call other functions, but we won't see that here.
...
We saw three types of queries: a range of times, within a bounding box, matching a level for a field. There are many more. Making an nice interface for that might be helpful.

(Had we downloaded the *entire* data set, we could just use subsetting and filtering to slice and dice the data R side, but here we want to filter our request, so use the SODA commands.)
...
Computer languages are now designed to minimize the taxing of the brain, if they can. In this case, we can use one function name to call 3 different functionalities and let the computer decide which *based* on the class of the value:
...
R has *many* different programming styles that implement polymorphism:

* S3 -- the oldest (early 90s). Easiest by far, but requires following conventions
* S4 -- more modern (early 00s). More robust, but more tedious
...
For example, "tab-completing" `plot` has these by *default*
...
* `default`: the catch all definitin
* `new`: actually shows how the naming is just a convention. There is no `new` class.

##

We should instead call `methods`:

```{r}
methods(plot)
```

## Okay, let's use this to simplify our soda calls:
...
`UseMethod` is the circus master that controls the dispatch.
...
Very similar -- but `val` and `var` are switched (a bit unnaturally!)

## `in`

Thinking in terms of vectorization, we might get passed a vector of values for val. Let's use the `in` feature of `SoQL`:
...
Soda.POSIXct <- function(from, var, to=NULL) {
    if (is.null(to)) {
        ...
        out <- sprintf("%s between '%s' and '%s'", var, posix_to_timestamp(from), posix_to_timestamp(to))
    }
    out
}
```
...
(We might also have written this using `from` as a length 1 or 2 vector. Then we would have something like:

```
sprintf("%s %s %s", var, ifelse(length(from) = 1, ">=", "between"), paste(shQuote(x), collapse=" and "))
```
...
What to do about the bounding box. We have no bounding box class. Can we make one?

Well it is *too* easy -- classes too are just a naming convention. Objects have an attribute `class`, a character vector. We can just push a name to the top. This *constructor*, `bbox`, creates a `BBOX` object:
...
Great, we could go back and *edit* each of our `Soda` functions to return a `Query` object, but instead we use a wrapper function. This also allows us to verify the name:
...
There is a package for querying [RSocrata](https://github.com/Chicago/RSocrata), but it is more concerned about getting the data, than constructing the query. There is a more robust means to download files there, but we will stick the magical `fromJSON`. But we would like to tidy up our downloaded files:
...
And then we can make a `request` with the following. (There is a bit more here than is needed.)
...
The hardest part is --- a good name! We will use the `the311`.

## Devtools

Package development in R is greatly assisted by the *devtools* package.
...
The latter is better for packages that may grow, the former maybe better as a package is being developed.

`the311` uses the latter. (see)
...
The `devtools` package gives directives which can make one file depend on another, hence the dependent file will load later. Looking at the example we have lines like:
...
Packages must be loaded. One development cycle is: load the package, edit the package, install the package, restart R, load the package ...

This can get tedious. `Devtools` provides `load_all` to temporarily install the package and avoid restarting R.
...
As exported functions can *mask* other packages functions or names, package developers *should* be conservative in what they export, and should re-use common names. (Such as `plot` for the default plot.)

Marking a function for export can be done using `roxygen` with the `@export` directive.

## roxygen

After mentioning it twice, `roxygen` gives a relatively simple way to document the functions in your package so that it is accessible to users of the package. The format is fairly simple:

* document what the function does

* document arguments (parameters)

* document if it is exported
