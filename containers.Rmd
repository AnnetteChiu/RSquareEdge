...
> A vector is a container holding n items all of the same type. The
> items are ordered using 1-based indexing. Vectors may be constructed
> by combining them with the `c` function.
...
Vectors must consist of elements all of the same type. If we try to
construct one of a different type, the values will be coerced:
...
Vectors can have attributes. A common one is to give the values names:
...
numbers -- matrix. In R the `matrix` function can do so:
...
common table making functions: `table` and `xtabs`. The table command
takes two vectors:
